...
상품 정보: {product_info if product_info else "없음"}"""
            ...
            if content_length <= 2600:
                ...
                logger.warning(
                    f"본문 길이가 기준 초과: {content_length}자 (요구: 2600자 이하). 재시도 ({attempt+1}/{max_retries})"
                )
            ...
            logger.error(
                f"본문 생성 중 예외 발생: {e}. 재시도 ({attempt+1}/{max_retries})"
            )
            if attempt < max_retries - 1:
                time.sleep(1)
            continue
    ...
    for attempt in range(max_retries):
        try:
            ...
            if 2100 <= summary_length <= 2300:
                ...
                logger.warning(
                    f"요약본 길이가 최소 기준 미달: {summary_length}자 (요구: 2100~2300자). 재시도 ({attempt+1}/{max_retries})"
                )
            elif 2300 < summary_length <= 2350:
                logger.warning(
                    f"요약본 길이가 최대 기준 초과: {summary_length}자 (요구: 2100~2300자). 재시도 ({attempt+1}/{max_retries})"
                )
            else:
                logger.warning(
                    f"요약본 길이가 범위를 벗어남: {summary_length}자 (요구: 2100~2300자). 재시도 ({attempt+1}/{max_retries})"
                )
            ...
            logger.error(
                f"본문 요약 중 예외 발생: {e}. 재시도 ({attempt+1}/{max_retries})"
            )
            if attempt < max_retries - 1:
                time.sleep(1)
            continue
...
def validate_content(content: str) -> bool:
    ...
    if content_length < 2100 or content_length > 2300:
        logger.warning(
            f"본문 길이가 올바르지 않습니다. (현재: {content_length}자, 요구: 2100~2300자)"
        )
        return False
...
def generate_content(
    ...
    for attempt in range(max_retries):
        try:
            # 1. 본문 생성 (2600자 이하)
            content = generate_content_text(
                keyword, title, outline, product_info, max_retries=2
            )
            ...
            if content_length > 1800:
                ...
                logger.info(
                    f"원본 본문이 1800자 초과({content_length}자)이므로 요약 후처리 진행"
                )
                summarized = summarize_content(content, max_retries=2)

                if not summarized:
                    logger.warning(
                        f"본문 요약 실패. 재시도 ({attempt+1}/{max_retries})"
                    )
                    if attempt < max_retries - 1:
                        time.sleep(2)
                    continue
                ...
                logger.info(
                    f"원본 본문이 1800자 이하({content_length}자)이므로 요약 후처리 건너뜀"
                )
            ...
            logger.error(
                f"본문 생성 프로세스 중 예외 발생: {e}. 재시도 ({attempt+1}/{max_retries})"
            )
            if attempt < max_retries - 1:
                time.sleep(2)
            continue
    ...
    logger.error(
        f"본문 생성 최종 실패: 최대 재시도 횟수({max_retries}) 초과. 폴백 본문 사용"
    )
