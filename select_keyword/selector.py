...
def load_keywords(
    csv_path: Path,
    limit: Optional[int] = MAX_CANDIDATES,
...
class KeywordSelector:
    ...
    def select(self, keywords: Sequence[str], max_retries: int = 2) -> SelectionResult:
...
def select_from_lists(
    keywords: List[str],
    exclude: List[str] | None = None,
    *,
    top_n: int = 10,
    limit: int = MAX_CANDIDATES,
    selector: Optional[KeywordSelector] = None,
) -> List[SelectionResult]:
    """
    이미 로드된 키워드 리스트와 제외 리스트를 받아 GPT로 최대 top_n개까지 선택한다.
    """
    selector = selector or KeywordSelector()
    candidates = _normalize_list(keywords)
    excluded = {item.strip().lower() for item in (exclude or []) if item.strip()}

    filtered = [kw for kw in candidates if kw.lower() not in excluded]
    if not filtered:
        raise ValueError("사용 가능한 후보 키워드가 없습니다.")
    ...
    while available and len(results) < top_n:
        try:
            result = selector.select(available)
        except Exception as exc:
            logger.error("키워드 선택 실패: %s", exc)
            raise
        results.append(result)
        # 다음 선택 시 중복되지 않도록 제외
        selected_normalized = result.keyword.strip().lower()
        available = [kw for kw in available if kw.lower() != selected_normalized]
