...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--max-keywords", type=int, default=50, help="최대 키워드 수 (후보 제한)")
    parser.add_argument(
        "--format",
        choices=("json", "text"),
        default="json",
...
def serialize_result(result: SelectionResult, output_format: str) -> str:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        keywords = load_keywords(args.keywords_csv, limit=args.max_keywords, exclude_path=args.exclude_csv)
        selections = select_from_lists(keywords, exclude=[], top_n=1, limit=args.max_keywords, selector=selector)
        result = selections[0]
